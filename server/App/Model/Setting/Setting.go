...
type Setting struct {
	...
	Message      string `json:"message"`                       // 消息 enable 启用、  un_enable 不启用
	ScanToUrl    string `json:"scan_to_url"`                   // 扫码跳转下载页 必须http起头
	ChangeQr     string `json:"change_qr"`                     // 扫码提醒 enable 启用、  un_enable 不启用
	MessageSound string `json:"message_sound"`                 // 消息提示音 enable 启用、  un_enable 不启用
	OnlineSound  string `json:"online_sound"`                  // 上线提示音 enable 启用、  un_enable 不启用
	Code         string `json:"code" gorm:"index:code_id_idx"` //客服Code
}
